...
get(Component,From,How):-  
   assumed(contains(From,Component),How)->true
;  delay_get(From,Component,How).
...
delay_get(Component,Container,How):-
   when(put(Component,Container,How),
        get(Component,Container,How)).
...
run(Goal):-
  assumel(waiting_thread(Goal)).
 
notify:-
  assumed(waiting_thread(Goal)),!,
  Goal.
notify.
...
when(Goal,Action):-
  ...
  Action.
when(Goal,Action):-
  run(when(Goal,Action)).
  
schedule(Main):-
  assumel(waiting_thread(Main)),
  schedule_threads.
