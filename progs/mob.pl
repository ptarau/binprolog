...
get_cc(NewCont,Cont)::-strip_cont(Cont,_,NewCont,NewCont).

% passes Closure to be called on accumulated cont

bug:-
  call_to(M,remote_run),
    % a(X),b(X),write(X),nl,
    write(hi),
  return(M),
  pp_clause(back).
...
call_to(Marker,Closure):-
  get_cont(Cont),
  get_goals1(Marker,(_,_,_,Cs),Cont,NewCont), % first _
